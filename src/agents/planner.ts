...
Use \`webSearchTool\` tool to gather information before planning.

# Steps

1. **Background investigation**: Use \`webSearchTool\` tool to gather information about the user's request.
...
Based on the gathered information from \`webSearchTool\` tool, provide the research plan in a structured format, such as:
...
- Focus on using webSearchTool and webCrawlTool tools effectively to gather credible and diverse information.
...
export const planner = createReactAgent({
    name: "planner",
    llm: chatModel,
    tools: [webSearchTool],
    prompt: prompt,
